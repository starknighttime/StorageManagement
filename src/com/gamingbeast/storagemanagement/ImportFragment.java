...
public class ImportFragment extends Fragment {
	...
	private ArrayList<ImportFragment.Product> mProducts = new ArrayList<ImportFragment.Product>();
	private ArrayList<ImportFragment.Product> mSelectedProducts = new ArrayList<ImportFragment.Product>();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View _contentView = inflater.inflate(R.layout.fragment_import,
				container, false);
		...
		mSource = (Spinner) _contentView.findViewById(R.id.sp_select_source);
		...
		ArrayAdapter<Source> _mSources = new ArrayAdapter<Source>(
				this.getActivity(), android.R.layout.simple_spinner_item);
		Cursor _c = DBHelper.getDatabase().rawQuery(
				"select * from source order by name", null);
		while (_c.moveToNext()) {
			_mSources.add(new Source(_c.getInt(_c.getColumnIndex("SKEY")), _c
					.getString(_c.getColumnIndex("name"))));
		}
		_c.close();
		_mSources
				.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
		mSource.setAdapter(_mSources);
			...
			Cursor _c2 = DBHelper
					...
					.rawQuery(
							...
							new String[] { _c.getInt(_c.getColumnIndex("brand"))
									+ "" });
			_c2.moveToNext();
			ImportFragment.Product temp = new ImportFragment.Product(
	...
	private void importProducts() {
		int _j = mSelectedProducts.size();
		DBHelper.getDatabase().execSQL(generateQueryOrder());
		Cursor _c = DBHelper.getDatabase().rawQuery(
				"select max(OKEY) from import_order", null);
		_c.moveToNext();
		...
		for (int _i = 0; _i < _j; _i++) {
			...
			DBHelper.getDatabase().execSQL(
					generateQueryOrderDetail(_OKEY, mSelectedProducts.get(_i)));
		}
		Toast.makeText(getActivity(), "进货成功", Toast.LENGTH_SHORT).show();
		mSelectedProducts.clear();
		mProducts.clear();
		mOrderPanel.invalidate();
		mProductsPanel.invalidateViews();
	}
	...
	private String generateQueryOrder() {
		return new StringBuffer("insert into import_order (source) values (")
				.append(((Source) mSource.getSelectedItem()).SKEY).append(")")
				.toString();
	}

	private String generateQueryOrderDetail(int OKEY, ImportFragment.Product p) {
		return new StringBuffer(
				"insert into import_detail (order_k,product,quantity,price) values (")
				.append(OKEY).append(",").append(p.PKEY).append(",")
				.append(p.amount).append(",").append(p.imPrice).append(")")
				.toString();
	}
	...
	private class ProductAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView,
			...
			final ImportFragment.Product _temp = mProducts.get(position);
	...
	private class OrderAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView,
			...
			final ImportFragment.Product _temp = mSelectedProducts
					.get(position);
			_name.setText(_temp.name);
